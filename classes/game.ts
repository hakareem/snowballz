...
export class Game {
    ...
    if (this.anyPlayers()) {
      ...
      if (Vector.distanceBetween(p.position, p.target) < 40) {
        ...
        let payload = {
          ...
          gameId: this.id,
          params: { destination: p.target, position: p.position, health: p.hp },
        };
  ...
  async createAndJoinServerGame(playerName: string) {
    ...
    let cmd = {
      cmd: "createGame",
      playerName: this.myName,
      params: { trees: this.obstacles, walk: this.nonObstacle },
    };
